...
struct task_struct g_ping_task;
...
void ping_test(void *param)
{
	...
	ping_buf = rtw_malloc(ping_size);
    if(NULL == ping_buf){
        printf("\n\r[ERROR] %s: Allocate ping_buf failed",__func__);
        return;
    }

    reply_buf = rtw_malloc(ping_size);
    if(NULL == reply_buf){
        rtw_free(ping_buf);
	...
	for(i = 0; (i < ping_count) || (infinite_loop == 1); i ++) {
		...
		if (inet_aton(host, &to_addr.sin_addr) == 0) {
			...
			if(server_host == NULL){
				...
				close(ping_socket);
				rtw_msleep_os(ping_interval * 1000);
				continue;
			}
		...
		if((reply_size = recvfrom(ping_socket, reply_buf, ping_size, 0, (struct sockaddr *) &from_addr, (socklen_t *) &from_addr_len))
			...
			if((pecho->id == PING_ID) && (pecho->seqno == htons(ping_seq))) {
				printf("\n\r[%s] %d bytes from %s: icmp_seq=%d time=%d ms", __FUNCTION__, reply_size - sizeof(struct icmp_echo_hdr), inet_ntoa(from_addr.sin_addr), htons(pecho->seqno), (reply_time - ping_time) * 1);
				ping_received_count++;
				ping_total_time += (reply_time - ping_time) * 1;
...
void cmd_ping_ameba(int argc, char **argv)
{
    int argv_count = 2;
	char * host;
    ...
    while(argv_count<=argc){
        //first operation
        if(argv_count == 2){
            host = rtw_malloc(strlen(argv[argv_count-1]) + 1);
...
void do_ping_test(char *ip, int size, int count, int interval)
{
	...
	if(ip == NULL){
		host = rtw_malloc(strlen(PING_IP) + 1);
	...
	if (rtw_create_task(&g_ping_task, "ping_test", STACKSIZE, 1, ping_test, NULL) != _SUCCESS)
		printf("\n\rERROR: Create ping_test failed.");
