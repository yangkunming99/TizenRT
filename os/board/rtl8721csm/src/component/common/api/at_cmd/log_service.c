...
//#elif defined(__CC_ARM) || defined(__GNUC__)
	...
	rtw_init_sema((_sema *)&log_rx_interrupt_sema, 1);
	rtw_down_timeout_sema((_sema *)&log_rx_interrupt_sema, 1);
#if CONFIG_LOG_SERVICE_LOCK
...
void log_service(void *param)
{
	...
	_AT_DBG_MSG(AT_FLAG_COMMON, AT_DBG_INFO, "\n\r# ");        
	while(1){
		while(rtw_down_timeout_sema((_sema *)&log_rx_interrupt_sema, RTW_MAX_DELAY) != _SUCCESS);
#if CONFIG_LOG_SERVICE_LOCK
		log_service_lock();
#endif
...
void start_log_service(void)
{
	struct task_struct CreatedTask;
	int result;
	...
	result = xTaskGenericCreate(
			...
			NULL);
#else		
	result = rtw_create_task( &CreatedTask, "log_service", STACKSIZE, 5,  log_service, NULL);
#endif
